...
export default function MenuDesplegable({ value, onChange, options, minWidth = '320px' }: MenuDesplegableProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div role="listbox" className="absolute top-full right-0 mt-3 w-full bg-[#0b1120]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-3 shadow-[0_0_50px_rgba(0,0,0,0.7)] z-dropdown anim-fade-in flex flex-col gap-2">
          {options.map((opc) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 py-2 px-4 rounded-xl cursor-pointer text-sm font-bold transition-all duration-300 border text-left ${
                  isActivo
