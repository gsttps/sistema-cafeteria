...
function SelectorPremium({ value, opciones, onChange, className = '', anchoPopup = 'w-[200px]' }: SelectorPremiumProps) {
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      {mostrarPopup && (
        <div className={`absolute top-full right-0 ${anchoPopup} bg-[#0b1120]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-3 mt-3 z-50 shadow-[0_0_50px_rgba(0,0,0,0.7)] anim-fade-in flex flex-col gap-2`}>
          {opciones.map((opcion) => {
